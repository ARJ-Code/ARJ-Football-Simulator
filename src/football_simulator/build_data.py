...
def conf_game(home_n: str, away_n: str):
    ...
    home_a = TeamAgent(
        home_n, Manager(LineUpSimulateStrategy(), ActionRandomStrategy(), 'H'), {p.dorsal: Player(50, p.dorsal, 'H', RandomStrategy()) for p in home})
    away_a = TeamAgent(
        away_n, Manager(LineUpSimulateStrategy(), ActionRandomStrategy(), 'A'), {p.dorsal: Player(50, p.dorsal, 'A', RandomStrategy()) for p in away})
